...
public class CartFragment extends Fragment implements CartAdapter.OnCartItemActionListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        // itens do carrinho
        cartItems = CartController.getInstance().getCartItems();
    ...
    public void onDecreaseQuantity(CartItem item) {
        if (item.getQuantity() > 1) {
            ...
            CartController.getInstance().removeItem(item);
        }
        cartAdapter.notifyDataSetChanged();
        updateTotal();
    }
    ...
    private void updateTotal() {
        double total = CartController.getInstance().getTotal();
    ...
    private void performCheckout() {
        ...
        // Criar o pedido
        double totalPrice = CartController.getInstance().getTotal();
